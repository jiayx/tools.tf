...
function showError(msg: string) {
  ...
  if (resultArea) {
    resultArea.innerHTML = '<div class="result-placeholder">等待输入…</div>'
...
function showResult({
  ...
  resultArea.innerHTML = `
    <div class="result-main">
      ...
      <div class="result-zone">${targetZone} · ${targetOffset}</div>
    </div>
    <div class="result-source">
      <span class="result-source-label">原始</span>
      <span>${sourceTime} · ${sourceZone} · ${sourceOffset}</span>
    </div>
    <div class="result-parsed">解析片段：「${parsedText}」</div>
  `
